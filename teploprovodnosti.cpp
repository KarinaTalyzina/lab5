    ...
    void solve() {  
        ...
        while (time < t_end) {
            ...
            for (int i = 1; i < N - 1; ++i) {
                ...
                double ci = ai; // Оптимизация: ai и ci равны
...
int main() {
    ...
    std::cout << "Введите плотность, rho (кг/м^3): ";
        std::cin >> rho;
    std::cout << "Введите удельную теплоемкость, c (Дж/(кг*К)): ";
    std::cin >> c;
    std::cout << "Введите начальную температуру, T0 (°C): ";
    std::cin >> T0;
    std::cout << "Введите температуру на левом краю, Tl (°C): ";
    std::cin >> Tl;
    std::cout << "Введите температуру на правом краю, Tr (°C): ";
    std::cin >> Tr;

    // Создаем объект класса HeatConduction1D и решаем задачу
